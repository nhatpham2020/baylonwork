...
export class BaseExportPreviewwhite implements IExportPreview {
    ...
    enablePreview(mesh: BABYLON.Mesh, previewOptions: PreviewOptions) {
        ...
        if (previewOptions.camera.upvec) {
            this.camera.upVector = previewOptions.camera.upvec;
        }

        if (previewOptions.position) {
            mesh.position = previewOptions.position;
        }

        this.enabled = true;
        this.currentObject = mesh;

        this.scene.clearColor = new BABYLON.Color4(255, 255, 255, 1);
